<script lang="ts">
  import { type PerkStats, PerksAnalyticsApiService } from '$lib/api/analytics';
  import { Bar } from 'svelte-chartjs';
  import { writable } from 'svelte/store';
  import ChartLayout from './ChartLayout.svelte';
  import { backgroundColor } from './chart-data';
  import { perkToString } from '$lib/util/enum-to-text';

  export let serverId: number;

  const now = new Date();
  const year = new Date().setUTCFullYear(now.getUTCFullYear(), 0, 1);
  const month = new Date().setUTCFullYear(
    now.getUTCFullYear(),
    now.getUTCMonth(),
    1
  );
  const week = now.getTime() - 604800 * 1000;

  const options = [
    {
      title: 'Year',
      date_from: new Date(year),
      date_to: new Date(),
    },
    {
      title: 'Month',
      date_from: new Date(month),
      date_to: new Date(),
    },
    {
      title: 'Week',
      date_from: new Date(week),
      date_to: new Date(),
    },
  ];

  const selectedOption = writable(2);

  function onClickOption(index: number) {
    selectedOption.set(index);
  }

  const data = writable<PerkStats[]>([]);

  async function fetch() {
    const { title, ...params } = options[$selectedOption];

    try {
      const { data: res } = await PerksAnalyticsApiService.getPlayTime({
        server_id: serverId,
        ...params,
      });

      data.set(res.sort((a, b) => b.count - a.count));
    } catch (error) {}
  }

  selectedOption.subscribe(() => fetch());
</script>

<ChartLayout>
  <svelte:fragment slot="title">Playtime By Perk</svelte:fragment>
  <svelte:fragment slot="tabs">
    <ul>
      {#each options as option, index (option.title)}
        <li class:selected={$selectedOption === index}>
          <div
            on:click={() => onClickOption(index)}
            on:keypress={(e) => e.code === 'Enter' && onClickOption(index)}
            role="button"
            tabindex="0"
          >
            {option.title}
          </div>
        </li>
      {/each}
    </ul>
  </svelte:fragment>
  <svelte:fragment slot="chart">
    {#if $data.length}
      <Bar
        options={{
          responsive: true,
          indexAxis: 'y',
        }}
        height={'250px'}
        data={{
          labels: $data.map((x) => perkToString(x.perk)),
          datasets: [
            {
              data: $data.map((x) => x.count),
              label: 'Playtime, hours',
              backgroundColor,
            },
          ],
        }}
      />
    {:else}
      No results
    {/if}
  </svelte:fragment>
</ChartLayout>

<style>
  ul {
    display: flex;
    flex-direction: row;
    gap: 1rem;
  }

  li {
    font-weight: bold;
  }

  .selected {
    border-bottom: 2px solid var(--text-primary);
  }
</style>
